...
This repository uses the project harness in `.ai/`. Codex must operate that harness automatically; the user should only need to describe work normally. Read `.ai/README.md` before repository-changing work.

If an active task exists, resume it when it matches the user's request, then compile its context for codex. For a new repository-changing request, internally create the task with objective and acceptance criteria, record a focused plan, and compile context before inspecting implementation files.
